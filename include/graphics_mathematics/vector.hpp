...
template<int N> constexpr vec<N> operator*(vec<N> v, float s) {
    for (int i = N; i--; v[i] *= s)
        ;
    return v;
}
...
template<int N> constexpr vec<N> operator*(float s, vec<N> v) { return v * s; }
...
template<int N> constexpr float operator*(const vec<N>& a, const vec<N>& b) {
    float sum = 0;
...
// L2 norm: ||v||
template<int N> float norm(const vec<N>& v) { return std::sqrt(v * v); }
